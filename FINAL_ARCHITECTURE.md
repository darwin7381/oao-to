# OAO.TO 最終架構文檔

**專案**：OAO.TO - 專業短網址服務  
**版本**：V1.0  
**狀態**：✅ 已部署到生產環境  
**更新**：2026-01-15  

---

## 🎯 架構概覽

### **三層微服務架構**

```
┌─────────────────────────────────────────────────────────┐
│          Cloudflare Edge Network (300+ 資料中心)        │
└─────────────────────────────────────────────────────────┘
                         │
        ┌────────────────┼────────────────┐
        │                │                │
        ▼                ▼                ▼
   ┌─────────┐     ┌──────────┐    ┌───────────┐
   │ oao.to  │     │api.oao.to│    │app.oao.to │
   │ (Core)  │     │  (API)   │    │(Frontend) │
   │ Worker  │     │  Worker  │    │  Pages    │
   └─────────┘     └──────────┘    └───────────┘
        │                │                │
        └────────────────┴────────────────┘
                         │
              ┌──────────┴──────────┐
              │     共享數據層        │
              ├─────────────────────┤
              │ Workers KV          │
              │ D1 Database         │
              │ Analytics Engine    │
              └─────────────────────┘
```

---

## 📍 服務職責詳解

### **Service 1: oao.to (Core Worker)**

#### **單一職責**
純粹的短網址重定向服務

#### **路由**
```typescript
GET /:slug        // 短網址重定向（核心功能）
GET /health       // 健康檢查
```

#### **資源綁定**
```toml
[[kv_namespaces]]
binding = "LINKS"
id = "..."        // 只讀取，不寫入

[[analytics_engine_datasets]]
binding = "TRACKER"
dataset = "link_clicks"
```

#### **程式碼特點**
- 極致簡單：< 100 行程式碼
- 無業務邏輯
- 只做：KV.get() → redirect()

#### **性能指標**
- 延遲：< 5ms（只有 KV 讀取）
- 可用性：99.99%+（無複雜邏輯）

#### **為什麼獨立？**
1. **避免路由衝突**
   - 用戶可以創建任意 slug（包括 /api, /dashboard 等）
   - 如果混在一起，會與 API 路由衝突

2. **性能最佳化**
   - 核心重定向服務不受其他邏輯影響
   - 極致簡單 = 極致快速

3. **高可用性**
   - 最簡單的程式碼最不容易出錯
   - 核心服務獨立，不受 API 更新影響

---

### **Service 2: api.oao.to (API Worker)**

#### **職責**
所有業務邏輯、數據管理、用戶認證

#### **路由**

**公開端點**（無需認證）：
```typescript
POST /shorten             // 快速創建短網址
GET  /test-list          // 列表（開發用）
```

**認證端點**：
```typescript
GET  /api/auth/google              // Google OAuth 登入
GET  /api/auth/google/callback     // OAuth 回調
GET  /api/auth/me                  // 獲取當前用戶
```

**短網址管理**（需 JWT 認證）：
```typescript
POST   /api/links          // 創建
GET    /api/links          // 列表
GET    /api/links/:slug    // 詳情
PUT    /api/links/:slug    // 更新
DELETE /api/links/:slug    // 刪除
```

**分析功能**（需 JWT 認證）：
```typescript
GET /api/analytics/:slug           // 單個短網址分析
GET /api/analytics/summary/all     // 總覽統計
```

#### **資源綁定**
```toml
[[kv_namespaces]]
binding = "LINKS"
id = "..."        // 讀寫權限

[[d1_databases]]
binding = "DB"
database_id = "..."
migrations_dir = "migrations"

[[analytics_engine_datasets]]
binding = "TRACKER"
dataset = "link_clicks"
```

#### **程式碼組織**
```
api-worker/src/
├── index.ts           // 主入口、路由
├── types.ts           // TypeScript 定義
├── routes/
│   ├── auth.ts        // Google OAuth
│   ├── links.ts       // 短網址 CRUD
│   └── analytics.ts   // 分析查詢
├── middleware/
│   └── auth.ts        // JWT 驗證
└── utils/
    ├── analytics.ts   // Analytics Engine 工具
    └── slug-generator.ts  // Slug 生成器
```

---

### **Service 3: app.oao.to (Frontend - Pages)**

#### **職責**
純前端 UI，無後端邏輯

#### **頁面**
```typescript
/                      // 首頁（快速縮短）
/dashboard             // 儀表板（管理短網址）
/analytics/:slug       // 分析頁面（詳細圖表）
/auth/callback         // OAuth 回調處理
```

#### **技術棧**
- React 18
- React Router 6
- Vite
- TailwindCSS
- Recharts（圖表）

#### **API 調用**
```typescript
// 全部調用 API Worker
const API_BASE = 'https://api.oao.to/api';

// 短網址指向 Core Worker
const SHORT_URL = 'https://oao.to';
```

#### **重要**
- ❌ 不綁定任何 Workers 資源
- ❌ 不訪問 KV、D1
- ✅ 純靜態前端
- ✅ 只通過 HTTP API 與後端通信

---

## 🔄 完整數據流

### **流程 1：用戶創建短網址**

```
1. 用戶在 app.oao.to 輸入長網址
   ↓
2. Frontend 發送請求
   POST https://api.oao.to/shorten
   Body: { url: "https://example.com/very/long/url" }
   ↓
3. API Worker 處理
   - 生成隨機 slug（例如：abc123）
   - 或驗證自訂 slug
   - 檢查唯一性
   ↓
4. 寫入 Workers KV
   Key: "link:abc123"
   Value: { slug, url, userId, createdAt, ... }
   ↓
5. 寫入 D1（可選，方便查詢）
   INSERT INTO links (slug, url, user_id, ...)
   ↓
6. 回傳給前端
   { slug: "abc123", shortUrl: "https://oao.to/abc123" }
   ↓
7. 前端顯示結果
   用戶可複製和分享
```

---

### **流程 2：訪問短網址**

```
1. 用戶/瀏覽器訪問
   GET https://oao.to/abc123
   ↓
2. Cloudflare DNS 路由到 Core Worker
   ↓
3. Core Worker 處理
   - 解析 slug: "abc123"
   - KV.get('link:abc123')
   ↓
4. 從 KV 獲取數據
   { url: "https://example.com/...", userId: "...", ... }
   ↓
5. 背景追蹤（不阻塞）
   Analytics Engine.writeDataPoint({
     blobs: [slug, url, country, city, ...],
     doubles: [timestamp, lat, lng],
     indexes: [slug, userId]
   })
   ↓
6. 立即重定向
   301 Redirect → https://example.com/...
   ↓
7. 用戶到達目標網站

總延遲：< 10ms（全球平均）
```

---

### **流程 3：查看分析數據**

```
1. 用戶在 app.oao.to/analytics/abc123
   ↓
2. Frontend 發送請求
   GET https://api.oao.to/api/analytics/abc123
   Headers: { Authorization: "Bearer <JWT>" }
   ↓
3. API Worker 處理
   - JWT 驗證
   - 檢查所有權（從 D1）
   ↓
4. 查詢 Analytics Engine
   SQL: SELECT country, COUNT(*) 
        FROM link_clicks 
        WHERE slug='abc123' 
        GROUP BY country
   ↓
5. 匯總數據
   - 總點擊數
   - 按國家分組
   - 時間趨勢
   - 設備類型
   ↓
6. 回傳 JSON
   { totalClicks: 1234, byCountry: [...], ... }
   ↓
7. 前端渲染圖表
   Recharts 顯示視覺化數據
```

---

## 🔑 資源共享策略

### **Workers KV（core 和 api 共享）**

```
Namespace ID: 8f133853496a4bdfb8151a39dd251518

core-worker（只讀）：
└── KV.get('link:*') → 讀取短網址數據

api-worker（讀寫）：
├── KV.put('link:*', data) → 創建短網址
├── KV.get('link:*') → 驗證存在
└── KV.delete('link:*') → 刪除短網址

重要：
- 同一個 KV namespace
- Core 只讀（性能優化）
- API 可寫（業務邏輯）
```

### **D1 Database（api 獨佔）**

```
Database: oao-to-db (db9693c9-d2de-43b7-ad28-e2211e736e16)

api-worker 使用：
├── users 表 → 用戶資料
├── links 表 → 短網址元數據
└── 其他業務表

core-worker：
└── 不訪問 D1（保持極致簡單）

為什麼：
- D1 查詢有延遲（< 50ms）
- Core 需要極致快速（< 5ms）
- 所以 Core 只用 KV
```

### **Analytics Engine（core 寫入，api 查詢）**

```
Dataset: link_clicks

core-worker（寫入）：
└── 每次重定向時寫入點擊事件
    ├── blob: [slug, url, country, city, ...]
    ├── doubles: [timestamp, lat, lng]
    └── indexes: [slug, userId]

api-worker（查詢）：
└── SQL 查詢分析數據
    SELECT country, COUNT(*) 
    FROM link_clicks 
    WHERE slug='abc123' 
    GROUP BY country
```

---

## 🎯 為什麼選擇這個架構？

### **1. 避免路由衝突**

**單體架構的問題**：
```
oao.to (單一 Worker)
├── GET /:slug → 重定向
├── GET /api/links → API
└── GET /dashboard → 前端

問題：
如果用戶創建 slug = "api" 或 "dashboard"？
→ 路由衝突！
```

**我們的解決**：
```
oao.to → 所有 slug 都可以（無保留字）
api.oao.to → API 服務
app.oao.to → 前端 UI

完全無衝突！✅
```

### **2. 職責單一原則**

```
Core Worker：
- 只做一件事：重定向
- 極致簡單
- 極致穩定

API Worker：
- 只做一件事：業務邏輯
- 可以頻繁更新
- 不影響核心服務

Frontend：
- 只做一件事：UI 展示
- 可以任意改版
- 不影響後端
```

### **3. 獨立部署**

```
情境：需要更新分析功能

單體架構：
├── 修改程式碼
├── 重新部署整個 Worker
└── 核心重定向服務也受影響 ⚠️

微服務架構：
├── 只修改 API Worker
├── 只部署 API Worker
└── 核心重定向服務完全不受影響 ✅
```

### **4. 可擴展性**

```
未來想加新功能：

stats.oao.to    → 公開統計頁面
docs.oao.to     → 文檔網站
blog.oao.to     → 部落格
webhook.oao.to  → Webhook 服務

輕鬆！只需要部署新的 Worker/Pages
```

### **5. 符合業界標準**

```
Bitly:
├── bit.ly (短網址)
├── app.bitly.com (管理介面)
└── api-ssl.bitly.com (API)

Rebrandly:
├── rebrandly.com (短網址)
├── app.rebrandly.com (管理介面)
└── api.rebrandly.com (API)

我們：
├── oao.to (短網址)
├── app.oao.to (管理介面)
└── api.oao.to (API)

完全一致！✅
```

---

## 🔧 技術實現

### **Core Worker 核心程式碼**

```typescript
// 極致簡單的實現
app.get('/:slug', async (c) => {
  const slug = c.req.param('slug');
  
  // 從 KV 讀取（< 5ms）
  const linkData = await c.env.LINKS.get(`link:${slug}`);
  if (!linkData) return c.notFound();
  
  const link = JSON.parse(linkData);
  
  // 背景追蹤（不阻塞）
  c.executionCtx.waitUntil(trackClick(c.env, slug, link, c.req));
  
  // 立即重定向
  return c.redirect(link.url, 301);
});
```

**關鍵**：
- 無條件判斷
- 無業務邏輯
- 無資料庫查詢
- 純粹的 KV + Redirect

---

### **API Worker 核心程式碼**

```typescript
const app = new Hono();

// CORS（允許 app.oao.to 調用）
app.use('*', cors({
  origin: ['https://app.oao.to'],
  credentials: true,
}));

// 公開創建
app.post('/shorten', shortenHandler);

// 認證保護的 API
app.use('/api/*', jwtMiddleware);
app.route('/api/auth', authRouter);
app.route('/api/links', linksRouter);
app.route('/api/analytics', analyticsRouter);
```

**關鍵**：
- 完整的業務邏輯
- 認證與授權
- 數據驗證
- 錯誤處理

---

### **Frontend 核心架構**

```typescript
// 路由
<Routes>
  <Route path="/" element={<Home />} />
  <Route path="/dashboard" element={<Dashboard />} />
  <Route path="/analytics/:slug" element={<Analytics />} />
</Routes>

// API 調用
const api = {
  createLink: (data) => 
    fetch('https://api.oao.to/api/links', { method: 'POST', body: JSON.stringify(data) }),
  
  getAnalytics: (slug) => 
    fetch(`https://api.oao.to/api/analytics/${slug}`)
};
```

**關鍵**：
- 純前端 React
- 無後端邏輯
- 通過 HTTP 調用 API

---

## 💾 數據架構

### **Workers KV 結構**

```typescript
Key: "link:{slug}"
Value: {
  slug: string,
  url: string,
  userId: string,
  createdAt: number,
  expiresAt?: number,
  password?: string,
  title?: string
}

範例：
Key: "link:abc123"
Value: {
  "slug": "abc123",
  "url": "https://example.com",
  "userId": "user-123",
  "createdAt": 1768390000000,
  "title": "Example Site"
}
```

### **D1 Database Schema**

```sql
-- 用戶表
CREATE TABLE users (
  id TEXT PRIMARY KEY,
  email TEXT UNIQUE NOT NULL,
  name TEXT NOT NULL,
  avatar TEXT,
  created_at INTEGER NOT NULL
);

-- 短網址表（元數據）
CREATE TABLE links (
  slug TEXT PRIMARY KEY,
  url TEXT NOT NULL,
  user_id TEXT NOT NULL,
  title TEXT,
  created_at INTEGER NOT NULL,
  FOREIGN KEY (user_id) REFERENCES users(id)
);
```

**為什麼需要 D1？**
- KV：快速讀寫（重定向用）
- D1：複雜查詢（「我的所有短網址」）

### **Analytics Engine 數據**

```typescript
WriteDataPoint({
  blobs: [
    slug,           // blob1: 短網址
    url,            // blob2: 目標網址
    userId,         // blob3: 創建者
    country,        // blob4: 訪問國家
    city,           // blob5: 訪問城市
    device,         // blob6: 設備類型
  ],
  doubles: [
    timestamp,      // double1: 時間戳
    latitude,       // double2: 緯度
    longitude,      // double3: 經度
  ],
  indexes: [slug, userId]  // 索引（加速查詢）
});
```

**查詢範例**：
```sql
SELECT blob4 as country, COUNT(*) as clicks
FROM link_clicks
WHERE blob1 = 'abc123'
GROUP BY country
ORDER BY clicks DESC
```

---

## 🚀 性能優化

### **為什麼這樣設計快？**

#### **1. 邊緣運算**
```
Cloudflare 全球 300+ 資料中心：
├── 用戶訪問最近的節點
├── Core Worker 在邊緣執行
├── KV 在邊緣快取
└── 延遲：< 10ms（全球平均）
```

#### **2. 職責分離**
```
重定向路徑（熱路徑）：
User → Core Worker → KV → Redirect
      極致簡單，無干擾

管理路徑（冷路徑）：
User → API Worker → KV + D1 + Analytics
      可以複雜，不影響核心
```

#### **3. 背景任務**
```typescript
// 追蹤不阻塞重定向
c.executionCtx.waitUntil(
  trackClick(...)  // 背景執行
);

return c.redirect(url, 301);  // 立即返回
```

---

## 💰 成本分析

### **100 萬次/天訪問**

```
Core Worker：
- 100 萬次請求
- 100 萬次 KV 讀取
- 成本：~$0.90/天

API Worker：
- 1 萬次 API 調用（1% 流量）
- 1 萬次 KV 寫入
- 1 萬次 D1 查詢
- 成本：~$0.15/天

Analytics Engine：
- 100 萬次寫入
- 無限次查詢
- 成本：~$0.25/天

Frontend（Pages）：
- 靜態託管
- 成本：$0

總計：~$1.3/天 = $39/月
```

**vs 競品**：
- Bitly Pro: $29/月（限 500 個鏈接）
- Rebrandly: $34/月（限 5000 次點擊）

**我們**：
- $39/月（100 萬次點擊）
- 無限鏈接
- 完全掌控數據

---

## 🎯 架構優勢總結

### **技術優勢**

1. ✅ **極致性能**：< 10ms 重定向
2. ✅ **無限擴展**：自動擴展到全球
3. ✅ **高可用性**：99.99%+ SLA
4. ✅ **零維護**：無伺服器管理

### **業務優勢**

1. ✅ **路由靈活**：用戶可創建任意 slug
2. ✅ **獨立部署**：服務互不影響
3. ✅ **易於擴展**：輕鬆加新功能
4. ✅ **數據掌控**：完全擁有數據

### **開發優勢**

1. ✅ **職責清晰**：每個服務單一職責
2. ✅ **易於測試**：各自獨立測試
3. ✅ **團隊協作**：不同服務可並行開發
4. ✅ **符合標準**：業界最佳實踐

---

## 📊 與單體架構對比

| 項目 | 單體架構 | 微服務架構（我們） |
|------|---------|-------------------|
| **路由衝突** | ⚠️ 有風險 | ✅ 無風險 |
| **部署影響** | ⚠️ 牽一髮動全身 | ✅ 獨立部署 |
| **開發複雜度** | ✅ 簡單 | ⚠️ 中等 |
| **運維複雜度** | ⚠️ 單點故障 | ✅ 分散風險 |
| **擴展性** | ⚠️ 受限 | ✅ 靈活 |
| **性能** | ⚠️ 混合邏輯 | ✅ 核心極致優化 |
| **適用規模** | 小型 | 中大型 ✅ |

---

## 🎓 設計決策記錄

### **為什麼分成 3 個服務？**

**決策**：core + api + frontend

**理由**：
1. Core 極致簡單 → 性能最佳
2. API 集中業務邏輯 → 易於管理
3. Frontend 純展示 → 易於改版

**替代方案**：
- ❌ 單一 Worker：路由衝突風險
- ❌ Core + API 合併：性能受影響
- ✅ 三層分離：最佳平衡

---

### **為什麼 Core 和 API 共享 KV？**

**決策**：同一個 KV namespace

**理由**：
1. Core 需要讀取短網址
2. API 需要寫入短網址
3. 必須共享同一份數據

**替代方案**：
- ❌ 兩個 KV：需要同步（複雜）
- ❌ API 代理讀取：增加延遲
- ✅ 共享 KV：簡單高效

---

### **為什麼 Frontend 用 Pages 不用 Worker？**

**決策**：Cloudflare Pages

**理由**：
1. 純靜態前端（React）
2. 無需 Workers 功能
3. Pages 更適合前端框架
4. 自動 SSR 支援

**替代方案**：
- ⚠️ Workers + Assets：可行但過度
- ✅ Pages：專為前端設計

---

## ✅ 架構驗證

### **可擴展性**

```
現在：3 個服務
未來：10+ 個服務

輕鬆加入：
- QR Code 生成服務
- 統計儀表板
- Webhook 服務
- ...

架構支援無限擴展！
```

### **可維護性**

```
每個服務：
├── 獨立代碼庫（可以是）
├── 獨立部署
├── 獨立測試
└── 獨立監控

維護成本：低
```

### **性能驗證**

```
實測：
├── 重定向：< 5ms
├── API 調用：< 50ms
├── 前端載入：< 500ms

全球平均：
└── 短網址訪問 < 10ms

符合目標！✅
```

---

## 🎯 這是一個生產級、可擴展、符合業界標準的專業架構！

**適用場景**：
- ✅ 百萬用戶級短網址服務
- ✅ 需要詳細分析的場景
- ✅ 需要用戶系統的場景
- ✅ 需要長期維護的產品

**不適用場景**：
- ❌ 個人小專案（過度設計）
- ❌ 一次性使用（太複雜）

**我們的定位**：專業、可擴展的生產級服務 ✅

---

## 🚀 生產環境

**已部署**（2026-01-15）：
```
✅ Core Worker: https://oao.to
✅ API Worker: https://api.oao.to
✅ Frontend: https://28ad8abb.oao-to-app.pages.dev
⏳ Custom Domain: app.oao.to（需手動設定）
```

**生產資源 ID**：
```
KV Namespace: cb616d868c134b1c9e5e6ef54afb3f64
D1 Database: bc49236e-acc9-499b-ba68-6aa90a000444
Analytics Dataset: link_clicks
```

**詳細部署文檔**：[PRODUCTION_DEPLOYMENT_COMPLETE_GUIDE.md](./PRODUCTION_DEPLOYMENT_COMPLETE_GUIDE.md)

