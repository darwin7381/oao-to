...
CREATE TABLE IF NOT EXISTS credits_new (
  ...
  balance INTEGER NOT NULL DEFAULT 0,              -- 唯一真实余额
  purchased_balance INTEGER DEFAULT 0,             -- 来源追踪：购买的
  ...
  -- 订阅方案
  plan_type TEXT DEFAULT 'free',                   -- 关联 plans 表
  plan_renewed_at INTEGER,
  
  -- 月配额（Pool A - 每月池）
  -- monthly_quota 已删除，改为从 plans.monthly_credits 动态获取
...
SELECT 'Migration 0006 completed - subscription_balance and monthly_quota removed' as status;
